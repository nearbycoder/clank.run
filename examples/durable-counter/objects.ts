...
export const Counter = defineDurableObject({
  ...
  state: s.object({
    value: s.number({ integer: true }),
    lastChangedAt: s.nullable(s.number({ integer: true, min: 0 })),
  }),
  initial: () => ({ value: 0, lastChangedAt: null }),
  methods: ({ query, mutation }) => ({
    ...
    add: mutation({
      args: { amount: s.number({ integer: true, min: -1_000, max: 1_000 }) },
      returns: s.number({ integer: true }),
