...
| Provider components left security-sensitive activation ordering to every adapter | Added a complete provider service with pre-intent capsule verification, durable operation/generation/fence state, drain-before-stop, cleanup-before-rollback, deferred background work, ingress-last activation, exact retry, and restart reconciliation | Provider service lifecycle, restart, activation failure, undrained traffic, fence conflict, metadata permission, deferred Docker, and data discard tests |
...
| Account limits were operator-only and a payment webhook could become a second, weak authorization system | Added an optional provider-neutral catalog whose entitlement snapshots remain subordinate to explicit operator overrides; Stripe checkout is browser/CSRF gated and every signed event binds the durable attempt, account, plan/Price, session, customer, and subscription before monotonic application | Adapter raw-signature/fixed-origin/bounds tests, replay/conflict and out-of-order lifecycle tests, malformed custom-provider tests, CLI contract, SSR refresh, and mobile browser verification |
