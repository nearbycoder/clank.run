...
Supervisor and deploy locks are local process state, so the current control plane is single-instance. Child processes support trusted operation; Docker adds a constrained container boundary for mutually untrusted applications.
