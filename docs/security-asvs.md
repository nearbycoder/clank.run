...
| API and web services | Content-type enforcement, streaming UTF-8/JSON limits, same-origin checks, generic 500 responses, scoped MCP tools | Security, backend, auth, MCP, platform, and conformance tests | Configure allowed hosts, TLS, proxy trust, quotas, and edge limits |
