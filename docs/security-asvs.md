...
| API and web services | Content-type enforcement, streaming UTF-8/JSON limits, same-origin checks, generic 500 responses, scoped MCP tools, transactional workspace/month and project/minute ingress admission | Security, backend, auth, data-plane, platform, and conformance tests | Configure allowed hosts, TLS, proxy trust, quotas, private application ports, and edge limits |
| File handling | Containment and realpath checks, dotfile denial, symlink rejection, atomic local objects, bounded S3 responses, SigV4 payload signing, verified SHA-256, signed application capabilities | Node, deploy, object-storage, services, and conformance tests | Add malware/content scanning, bucket-scoped policy, lifecycle, and provider monitoring |
| Authentication | Scrypt passwords, generic failures, session expiry/idle expiry, verification, recovery, MFA, WebAuthn passkeys | Auth and WebAuthn tests | Configure auth-verification delivery, pepper/KMS, bot defense, and recovery policy |
| Platform invitations | Email-bound single-use tokens, closed-signup assisted registration, fragment links, encrypted transactional delivery outbox, leased/idempotent retry, ciphertext erasure | Platform invitation, restart, crash-lease, multi-control-plane, revoke-fencing, and service-driver tests | Verify the sender domain, protect provider credentials and invited mailboxes, monitor failed state |
| Hosted billing | Optional exact plan catalog, browser-only CSRF checkout/portal, durable idempotent attempts, provider/customer/subscription/Price binding, raw-body signed webhooks, replay/conflict and ordering controls, fail-closed entitlement snapshots, audited operator grants | Stripe adapter, billing configuration, platform lifecycle, malformed-provider, CLI, SSR, and responsive browser tests | Protect provider credentials/dashboard, configure tax/refund/dispute policy, alert on webhooks, and test the complete provider lifecycle |
...
| Tokens and secrets | High-entropy bearer tokens, hashes at rest, resource-bound MCP access tokens, rotating refresh families, AES-256-GCM platform secrets, redacted structured logs | MCP, platform, observability, services, conformance, and security-audit tests | Use external secret management, rotation, least privilege, grant review, and access review |
| CI workload identity | Fixed-issuer/JWKS GitHub OIDC, RS256, exact audience/repository ID/workflow/event/ref binding, replay digest, short pull-scoped token, secretless pinned workflows, isolation gate | OIDC verifier, platform preview-scope/replay/privacy, CLI transport/workflow, conformance, and workflow-policy tests | Protect base-branch workflow review, GitHub administrators, and isolated runner/provider capacity |
