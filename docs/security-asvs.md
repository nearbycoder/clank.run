...
| Secure communication | HTTPS-only external service drivers, exact ingress hosts, hop-header stripping, bounded proxying, verified DNS routing, restricted TLS eligibility | Data-plane, platform, and chaos tests | Supply TLS termination/key custody, HSTS, WAF, and egress policy |
