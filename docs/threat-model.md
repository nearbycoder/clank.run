...
| Object-storage compromise | Redirect credentials, alter stored bytes/metadata, return an oversized body, reuse broad bucket authority, or delete another environment's objects | HTTPS origin validation, SigV4 signed payload/headers, redirect refusal, strict keys, independent bounds and SHA-256, generic errors, optional namespace prefix | Bucket-scoped credentials/policy, environment separation, provider audit/retention, client-side encryption, growth/integrity alerts |
