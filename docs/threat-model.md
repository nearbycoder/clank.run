...
| Provider bridge takeover | Steal a provider token, replay a reconcile, downgrade a generation, smuggle a release, or leak a provider error | Separate high-entropy bearer, HTTPS/private-network guidance, fixed protocol, strict headers, bounded binary body, fresh artifact hash/config validation, no coordinator credentials, monotonic generation/fence contract, generic errors | Rotate/scoped provider tokens, network admission, durable adapter fencing, provider audit and isolation |
