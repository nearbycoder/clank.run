...
| Provider bridge takeover | Steal a provider token, replay a reconcile, downgrade a generation, smuggle a release/runtime, or leak a provider error | Separate high-entropy bearer, HTTPS/private-network guidance, fixed protocols, strict headers, bounded binary body, fresh artifact/capsule validation and desired-state binding, no coordinator credentials, monotonic generation/fence contract, generic errors | Rotate/scoped provider tokens, network admission, durable adapter fencing, provider audit and isolation |
| Remote ingress confusion | Send public traffic to an old generation, spoof provider binding headers, escape a provider route with a scheme-relative/encoded traversal path, race a late response, or expose its route token | Allowlisted origin, strict unencoded provider path, overwritten project/protocol/generation/token headers, generation-scoped and race-fenced circuits, identically bound private health, reserved response stripping, no token in URLs/metrics/public failures | Provider must validate the complete committed binding and isolate project runtimes; activation remains closed until publish/revoke and traffic-switch certification land |
...
| Provider data corruption or stale replay | Crash between database/metadata writes, replay an old fence, change a stored path, replace another project, follow a storage link, or lose rollback bytes during cleanup | Independent capsule/desired binding, per-project state, monotonic fence high-water mark, exact scoped metadata, owner-only real paths, SQLite integrity checks, apply/rollback journals, atomic state commit, one referenced safety snapshot, explicit confirmations | One writer per provider root, stateful node pinning, durable disks, isolated runtimes, encrypted independent backups, restore/deletion drills, and filesystem/operator security |
