...
| Invitation replay or disclosure | Reuse a superseded token, scrape pending addresses or audit metadata, flood active invitations | Hashed email-bound single-use tokens, atomic replacement/revocation, administrator-only pending metadata, developer audit redaction, 100-active cap, audit | Deliver tokens through a trusted channel and protect invited mailboxes |
